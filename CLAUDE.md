...
- AWS のリソース(EC2・RDS・セキュリティグループ)は、**Terraform**(`terraform/`)で作る。**`terraform apply`・`terraform destroy` の前に、必ず `terraform plan` の内容をユーザーに見せて確認する**(「無料ゲート」)。私(Claude)は、`apply`・`destroy` を、勝手に実行しない(ユーザーが実行する)。
- **AWS に関する作業の前に、`docs/07-デプロイガイド.md`(全5部)を必ず読む。** 詳細は `docs/06-技術スタック.md` も参照する。

## 秘密情報の扱い(このリポジトリは公開)

このリポジトリは、GitHub 上で**公開(public)**にしている。**パスワード・APIキー・秘密鍵などは、絶対に、コード・設定ファイルとして、コミットしない。**

- **Git に入れない**(`.gitignore` で除外): `backend/config/master.key`(Rails のマスターキー)、`.env` / `.env.local` / `.env.production` など(見本の `*.example` だけは、値を入れずに公開する)、`terraform/terraform.tfvars` と `terraform/*.tfstate*`(AWS の認証情報や、DB パスワードが関わる)、`*.pem`(SSH のキーペア)
- **秘密の値は、環境変数で渡す**: RDS のパスワードは `TF_VAR_db_password`、AWS の認証情報は `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`。コードやコミットには書かない
- 新しく秘密の値を扱うファイルを作るときは、**先に `.gitignore` に追加してから**、値を書く
- 2026年9月23日、公開する前に、`git log --all -p` で、過去のコミット履歴もふくめて、アクセスキー・パスワードなどの漏えいがないことを確認した(Terraform 関連ファイル、`.env`、`master.key`、`.pem` など)。**今後、秘密情報を扱うファイルを追加・変更したときは、コミット前に、同様の確認を行う**
