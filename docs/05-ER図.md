...
| status の値 | Rails の enum の名前 | 列(画面) | 優先度順ボタン | 習得日の表示 |
|---|---|---|---|---|
| 0 | `unlearned` | 未習得 | あり | なし |
| 1 | `learning` | 習得中 | あり | なし |
| 2 | `mastered` | 習得済み | なし | あり |
...
| 3 | 同じ状態の中では、スキルの並び順は 0 から連番になる |
| 4 | 優先度順の並べ替えは、未習得と習得中に対してだけ行える。同じ優先度は、元の並び順を保つ |
